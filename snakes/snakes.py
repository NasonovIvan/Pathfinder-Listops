...
def make_many_snakes(
    ...
    while isnake < num_snakes:
        ...
        while snake_retry_count <= max_snake_trial:
            curr_image, curr_mask, success = make_snake(
                curr_image,
                curr_mask,
                num_segments,
                segment_length,
                ...
                display_snake,
                display_segment,
                allow_shorter_snakes,
                stop_with_availability,
            )
            if success is False:
                snake_retry_count += 1
            else:
...
def make_snake(
    ...
    for isegment in range(1):
        ...
        (
            ...
            stop_with_availability=stop_with_availability,
        )
        if success is False:
...
def seed_snake(
    ...
    while trial_count <= max_segment_trial:
        ...
        _, struct = draw_line_n_mask(
            struct_shape,
            struct_head,
            sampled_orientation_in_rad,
            length,
            thickness,
            margin,
            large_dilation_structs,
        ...
        lined_mask[:, 0] = 1
        lined_mask[:, -1] = 1
        dilated_mask = binary_dilate_custom(lined_mask, struct, value_scale=1.0)
        # dilation in the same orientation as the tail

        # run coordinate searcher while also further dilating
        _, raw_num_available_coordinates = find_available_coordinates(
            np.ceil(mask - 0.3), margin=0
        )
        available_coordinates, num_available_coordinates = find_available_coordinates(
            np.ceil(dilated_mask - 0.3), margin=0
        )
        if (stop_with_availability is not None) and (
            np.float64(raw_num_available_coordinates) / (mask.shape[0] * mask.shape[1])
            < stop_with_availability
        ):
        ...
        sampled_tail = [
            available_coordinates[0][random_number],
            available_coordinates[1][random_number],
        ]  # CHECK OUT OF BOUNDARY CASES
        sampled_head = translate_coord(sampled_tail, sampled_orientation_in_rad, length)
        sampled_pivot = translate_coord(
            sampled_head, sampled_orientation_in_rad_reversed, length + margin
        )
        if (
            (sampled_head[0] < 0)
            | (sampled_head[0] >= mask.shape[0])
            | (sampled_head[1] < 0)
            | (sampled_head[1] >= mask.shape[1])
            | (sampled_pivot[0] < 0)
            | (sampled_pivot[0] >= mask.shape[0])
            | (sampled_pivot[1] < 0)
            | (sampled_pivot[1] >= mask.shape[1])
        ):
...
def extend_snake(
    ...
    # get temporarily dilated mask
    dilated_mask = binary_dilate_custom(mask, small_dilation_structs, value_scale=1.0)
    # get candidate endpoints
    unique_coords, unique_orientations, cmf, pmf = get_coords_cmf(
        new_pivot, last_orientation, length + margin, dilated_mask, continuity
    )
    # sample endpoint
    if pmf is None:
        ...
        while trial_count <= max_segment_trial:
            ...
            new_head = unique_coords[
                sampled_index, :
            ]  # find the smallest index whose value is greater than rand
...
def get_coords_cmf(last_endpoint, last_orientation, step_length, mask, continuity):
    ...
    deg_per_pixel = 360.0 / (2 * step_length * np.pi)
    ...
    samples_in_coord = np.concatenate(
        (np.expand_dims(samples_in_y, axis=1), np.expand_dims(samples_in_x, axis=1)),
        axis=1,
    ).astype(int)
    OOB_rows = (
        (samples_in_y >= height)
    ...
    unique_orientations = np.arctan2(
        unique_displacements[:, 0], unique_displacements[:, 1]
    )  # from -pi to pi
    unique_delta = np.minimum(
        np.abs(unique_orientations - last_orientation),
        2 * np.pi - np.abs(unique_orientations - last_orientation),
    )
    unique_delta = np.minimum(unique_delta * continuity, 0.5 * np.pi)
    unique_cosinedistweights = np.maximum(np.cos(unique_delta), 0) ** 2
    # compute probability distribution
    inverted = 1 - mask[unique_coords[:, 0], unique_coords[:, 1]]
    pmf = np.multiply(
        np.array([inverted]), unique_cosinedistweights
    )  # weight coordinates according to continuity
    total = np.sum(pmf)
    if total < 1e-4:
        # print('dead-end while expanding')
        return None, None, None, None
...
def draw_line_n_mask(
    ...
    miniline_blown_shape = (
        length + int(np.ceil(thickness)) + margin
    ) * 2 * aa_scale + 1
    ...
    miniline_blown_head = translate_coord(
        [miniline_blown_center, miniline_blown_center], orientation, length * aa_scale
    )
    miniline_blown_im = Image.new(
        "F", (miniline_blown_shape, miniline_blown_shape), "black"
    )
    line_draw = ImageDraw.Draw(miniline_blown_im)
    line_draw.line(
        [
            (miniline_blown_center, miniline_blown_center),
            (miniline_blown_head[1], miniline_blown_head[0]),
        ],
        fill="white",
        width=miniline_blown_thickness,
    )

    # resize with interpolation + apply contrast
    miniline_shape = (length + int(np.ceil(thickness)) + margin) * 2 + 1
    miniline_blown_im_pil = Image.fromarray(np.array(miniline_blown_im))
    miniline_im = miniline_blown_im_pil.resize(
        (miniline_shape, miniline_shape), Image.LANCZOS
    )
    miniline_im = np.array(miniline_im).astype(np.float32) / 255

    if contrast_scale != 1.0:
        miniline_im *= contrast_scale

    # draw a mask
    minimask_blown_im = binary_dilate_custom(
        miniline_blown_im, large_dilation_struct, value_scale=1.0
    ).astype(np.uint8)
    minimask_blown_im_pil = Image.fromarray(np.array(minimask_blown_im))
    minimask_im = minimask_blown_im_pil.resize(
        (miniline_shape, miniline_shape), Image.LANCZOS
    )
    minimask_im = np.array(minimask_im).astype(np.float32) / 255
    ...
    m_im = l_im.copy()
    l_im_vertical_range_raw = [
        start_coord[0] - (length + int(np.ceil(thickness)) + margin),
        start_coord[0] + (length + int(np.ceil(thickness)) + margin),
    ]
    l_im_horizontal_range_raw = [
        start_coord[1] - (length + int(np.ceil(thickness)) + margin),
        start_coord[1] + (length + int(np.ceil(thickness)) + margin),
    ]
    l_im_vertical_range_rectified = [
        np.maximum(l_im_vertical_range_raw[0], 0),
        np.minimum(l_im_vertical_range_raw[1], im_size[0] - 1),
    ]
    l_im_horizontal_range_rectified = [
        np.maximum(l_im_horizontal_range_raw[0], 0),
        np.minimum(l_im_horizontal_range_raw[1], im_size[1] - 1),
    ]
    miniline_im_vertical_range_rectified = [
        np.maximum(0, -l_im_vertical_range_raw[0]),
        miniline_shape
        - 1
        - np.maximum(0, l_im_vertical_range_raw[1] - (im_size[0] - 1)),
    ]
    miniline_im_horizontal_range_rectified = [
        np.maximum(0, -l_im_horizontal_range_raw[0]),
        miniline_shape
        - 1
        - np.maximum(0, l_im_horizontal_range_raw[1] - (im_size[1] - 1)),
    ]
    l_im[
        l_im_vertical_range_rectified[0] : l_im_vertical_range_rectified[1] + 1,
        l_im_horizontal_range_rectified[0] : l_im_horizontal_range_rectified[1] + 1,
    ] = miniline_im[
        miniline_im_vertical_range_rectified[0] : miniline_im_vertical_range_rectified[
            1
        ]
        + 1,
        miniline_im_horizontal_range_rectified[
            0
        ] : miniline_im_horizontal_range_rectified[1]
        + 1,
    ].copy()
    m_im[
        l_im_vertical_range_rectified[0] : l_im_vertical_range_rectified[1] + 1,
...
def binary_dilate_custom(im, struct, value_scale=1.0):
    # out = ndimage.morphology.binary_dilation(np.array(im), structure=struct, iterations=iterations)
    out = (
        np.array(
            cv2.dilate(np.array(im), kernel=struct.astype(np.uint8), iterations=1)
        ).astype(float)
        / value_scale
    )
...
def translate_coord(coord, orientation, dist, allow_float=False):
    ...
    if allow_float is True:
        ...
        new_coord = [
            int(np.ceil(coord[0] + y_displacement)),
            int(np.ceil(coord[1] + x_displacement)),
        ]
    return new_coord
...
def gray2red(im, bw="w"):
    if bw == "b":
        ...
        out = np.concatenate([im_expanded, padding], axis=-1)
    elif bw == "w":
        padding = np.ones((im.shape[0], im.shape[1], 3))
        im_expanded = np.tile(im, reps=(3, 1, 1))
        im_expanded = np.transpose(im_expanded, (1, 2, 0))
        im_expanded[:, :, 0] = 0  # Red channel
        out = padding - im_expanded
    return out
...
def test():
    ...
    num_distractor_paddles = int(33 * (9.0 / target_paddle_length))  # 4
    ...
    show1 = cv2.resize(red_target, (imsize, imsize), interpolation=cv2.INTER_LANCZOS4)
    plt.imshow(show1, vmin=0, vmax=1)
    plt.axis("off")

    plt.subplot(2, 1, 2)
    gray_total = gray2gray(1 - image3)
    show2 = cv2.resize(gray_total, (imsize, imsize), interpolation=cv2.INTER_LANCZOS4)
    plt.imshow(show2, vmin=0, vmax=1, cmap="gray")
    plt.axis("off")
...
def from_wrapper(args):
    ...
    if args.save_gt:
        ...
        os.makedirs(os.path.join(args.contour_path, gt_sub_path), exist_ok=True)
    ...
    while iimg < args.n_images:
        ...
        target_im, mask = make_many_snakes(
            image,
            mask,
            1,
            args.max_target_contour_retrial,
            args.contour_length,
            args.paddle_length,
            args.paddle_thickness,
            margin,
        ...
        if target_im is None:
            continue
        interm_im, mask = make_many_snakes(
            target_im,
            mask,
            args.num_distractor_contours,
            args.max_distractor_contour_retrial,
            args.distractor_length,
            args.paddle_length,
            args.paddle_thickness,
            margin,
        ...
        if args.use_single_paddles is not False:
            num_bits = (
                args.use_single_paddles
                - args.contour_length
                - args.distractor_length * args.num_distractor_contours
            )
            final_im, mask = make_many_snakes(
                interm_im,
                mask,
                num_bits,
                10,
        ...
        if args.save_images:
            fn = f"sample_{iimg}.png"
            # Convert float image to uint8 before saving
            final_im_save = (final_im * 255).clip(0, 255).astype(np.uint8)
            imageio.imwrite(
                os.path.join(args.contour_path, contour_sub_path, fn), final_im_save
            )

        if args.save_gt:
            fn = f"gt_{iimg}.png"
            # Convert float image to uint8 before saving
            target_im_save = (target_im * 255).clip(0, 255).astype(np.uint8)
            imageio.imwrite(
                os.path.join(args.contour_path, gt_sub_path, fn), target_im_save
            )

        if args.save_metadata:
            metadata = accumulate_meta(
                metadata, contour_sub_path, fn, args, iimg, paddle_margin=margin
            )
