...
def accumulate_meta_segment(
    array, contour_sub_path, seg_sub_path, filename, args, nimg, paddle_margin=None
):
    # NEW VERSION
    array += [
        [
...
def two_snakes(
    ...
    for isegment in range(1):
        (
            current_images,
            current_mask,
            current_segment_masks,
            current_pivots,
            current_orientations,
            origin_tips,
            success,
        ) = initialize_two_seeds(
            image_size,
            padding,
            seed_distance,
            segment_length,
        ...
        if success is False:
            return (
                np.zeros((image_size[0], image_size[1])),
                np.zeros((image_size[0], image_size[1])),
                None,
                None,
                False,
    ...
    for isegment in range(num_segments - 1):
        ...
        for isnake in range(len(current_segment_masks)):
            (
                current_images[isnake],
                current_mask,
                current_segment_masks[isnake],
                current_pivots[isnake],
                current_orientations[isnake],
                terminal_tips[isnake],
                success,
            ) = snakes.extend_snake(
                list(current_pivots[isnake]),
                current_orientations[isnake],
                current_segment_masks[isnake],
                current_images[isnake],
                current_mask,
                ...
                margin,
                continuity,
                contrast * snake_contrast_mu_list[isnake],
                small_dilation_structs,
                large_dilation_structs,
                aa_scale=aa_scale,
                display=display_segment,
...
def initialize_two_seeds(
    ...
    while trial_count <= max_segment_trial:
        sampled_orientation_in_rad1 = (
            np.random.randint(low=-180, high=180) * np.pi / 180
        )
        ...
        _, struct = snakes.draw_line_n_mask(
            struct_shape,
            struct_head,
            sampled_orientation_in_rad1,
            length,
        ...
        dilated_mask = snakes.binary_dilate_custom(lined_mask, struct, value_scale=1.0)
        # dilation in the same orientation as the tail

        # run coordinate searcher while also further dilating
        _, raw_num_available_coordinates = snakes.find_available_coordinates(
            np.ceil(mask - 0.3), margin=0
        )
        available_coordinates, num_available_coordinates = (
            snakes.find_available_coordinates(np.ceil(dilated_mask - 0.3), margin=0)
        )
        ...
        sampled_head1 = snakes.translate_coord(
            sampled_tail1, sampled_orientation_in_rad1, length
        )
        sampled_pivot1 = snakes.translate_coord(
            sampled_head1, sampled_orientation_in_rad_reversed, length + margin
        )
        sampled_tip1 = [sampled_tail1[0], sampled_tail1[1]]
        if (
            (sampled_head1[0] < 0)
            | (sampled_head1[0] >= mask.shape[0])
            | (sampled_head1[1] < 0)
            | (sampled_head1[1] >= mask.shape[1])
            | (sampled_pivot1[0] < 0)
            | (sampled_pivot1[0] >= mask.shape[0])
            | (sampled_pivot1[1] < 0)
            | (sampled_pivot1[1] >= mask.shape[1])
        ):
    ...
    l_im, m_im1 = snakes.draw_line_n_mask(
    ...
    while trial_count <= max_segment_trial:
        ...
        sample_in_y = int(
            np.round_(sampled_tail1[0] + (seed_distance * np.sin(sample_in_rad)))
        )
        sample_in_x = int(
            np.round_(sampled_tail1[1] + (seed_distance * np.cos(sample_in_rad)))
        )
        sampled_tail2 = [sample_in_y, sample_in_x]
        sampled_head2 = snakes.translate_coord(
            sampled_tail2, sampled_orientation_in_rad2, length
        )
        sampled_pivot2 = snakes.translate_coord(
            sampled_head2, sampled_orientation_in_rad_reversed, length + margin
        )
        sampled_tip2 = [sampled_tail2[0], sampled_tail2[1]]
        if (
            (sampled_head2[0] < 0)
            | (sampled_head2[0] >= mask.shape[0])
            | (sampled_head2[1] < 0)
            | (sampled_head2[1] >= mask.shape[1])
            | (sampled_pivot2[0] < 0)
            | (sampled_pivot2[0] >= mask.shape[0])
            | (sampled_pivot2[1] < 0)
            | (sampled_pivot2[1] >= mask.shape[1])
        ):
            # print('missampled seed +segment_trial_count')
            trial_count += 1
            continue
    ...
    l_im, m_im2 = snakes.draw_line_n_mask(
        (mask.shape[0], mask.shape[1]),
        sampled_tail2,
        sampled_orientation_in_rad2,
        length,
...
def draw_circle(window_size, coordinate, radius, aa_scale):
    ...
    y, x = np.ogrid[
        -coordinate[0] * aa_scale : (window_size[0] - coordinate[0]) * aa_scale,
        -coordinate[1] * aa_scale : (window_size[1] - coordinate[1]) * aa_scale,
    ]
    mask = x**2 + y**2 <= (radius * aa_scale) ** 2
    image[mask] = 1
    new_image = Image.fromarray(np.array(image))
    final_image = new_image.resize((window_size[0], window_size[1]), Image.LANCZOS)
...
def from_wrapper(args):
    ...
    while iimg < args.n_images:
        ...
        large_dilation_structs = snakes.generate_dilation_struct(
            margin * args.antialias_scale
        )

        ### SAMPLE TWO TARGET SNAKES
        success = False
        while not success:
            twosnakes, mask, origin_tips, terminal_tips, success = two_snakes(
                args.window_size,
                args.padding,
                args.seed_distance,
                args.contour_length,
        ...
        if args.use_single_paddles is not False:
            ...
            num_single_paddles = (
                total_num_paddles
                - 2 * args.contour_length
                - num_distractor_snakes * args.distractor_length
            )
        ...
        origin_circle = draw_circle(
            args.window_size,
            origin_mark_coord,
            args.marker_radius,
            args.antialias_scale,
        )
        terminal_circle = draw_circle(
            args.window_size,
            terminal_mark_coord,
            args.marker_radius,
            args.antialias_scale,
        ...
        if args.segmentation_task:
            marker = origin_circle.astype(np.float32)  # / 255
            merker2 = terminal_circle.astype(np.float32)  # / 255
            ...
            markers = np.maximum(origin_circle, terminal_circle).astype(
                np.float32
            )  # / 255
        ...
        image_marked = (
            (image_marked - image_marked.min())
            * (255.0 / (image_marked.max() - image_marked.min()))
        ).astype(np.uint8)

        if args.segmentation_task:
            if args.save_images:
                fn = f"sample_{iimg}.png"
                Image.fromarray(np.uint8(image_marked)).save(
                    os.path.join(args.contour_path, contour_sub_path, fn)
                )
                Image.fromarray(np.uint8(target_segment)).save(
                    os.path.join(args.contour_path, seg_sub_path, fn)
                )
            if args.save_metadata:
                metadata = accumulate_meta_segment(
                    metadata,
                    contour_sub_path,
                    seg_sub_path,
                    fn,
