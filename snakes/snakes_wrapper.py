...
class Args:
    def __init__(
        ...
        contour_length=15,
        distractor_length=5,
        num_distractor_contours=6,
        use_single_paddles=400,
...
dataset_root = os.path.join("./data", "new_128")
if not os.path.exists(dataset_root):
    try:
        os.makedirs(dataset_root, exist_ok=True)
        print(f"Created directory: {dataset_root}")
    except Exception as e:
        print(f"Error creating directory {dataset_root}: {str(e)}")
        sys.exit(1)
...
args.num_distractor_contours = int(33 * (9.0 / args.contour_length))
...
for cl in [6, 18]:
    ...
    args.num_distractor_contours = int(30 * (9.0 / args.contour_length))
    snakes.from_wrapper(args)
...
for nc in [10, 20, 30, 40]:
    ...
    args.contour_path = os.path.join(dataset_root, dataset_subpath)
    args.paddle_contrast_list = list(
        np.linspace(1.0, 1.0 - contrast_interval * nc, num=nc + 1, endpoint=True)
    )
    # snakes.from_wrapper(args)
    # Negative
